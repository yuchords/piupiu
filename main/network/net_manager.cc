...
NetManager::NetManager() {
        while(!instance_inited) {
            BLUFI_ERROR("WIFI Manager inited twice");
        }
    initialise_wifi();
    esp_blufi_controller_init();
...
esp_err_t NetManager::enter_blufi_mode() {
    enable_ble();
    esp_err_t ret;
...
esp_err_t NetManager::exit_blufi_mode() {
    esp_err_t ret;


    esp_blufi_adv_stop();
    if (ret != ESP_OK) {
        ESP_LOGE("BLUFI", "Failed to stop adv");
    }


    ret = esp_blufi_profile_deinit();
    if (ret != ESP_OK) {
        ESP_LOGE("BLUFI", "Failed to deinit profile");
        return ret;
    }

    ESP_LOGI("BLUFI", "BluFi service stopped, Bluetooth stack is still alive.");
    return ESP_OK;
}
esp_err_t NetManager::enable_ble() {
    int ret;
...
esp_err_t NetManager::disable_ble() {
    esp_err_t ret;
